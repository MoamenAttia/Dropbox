...
def handle_message(msg, socket, success_socket):
...
def node_keeper_client(node_ip, node_port):
    ...
    while True:
        ...
        reply = handle_message(msg, socket, success_socket)
        socket.send_pyobj(reply)


def main():
    Process(target=node_keeper_client, args=(NODE_KEEPER_IP_1, NODE_KEEPER_CLIENT_REP_1,)).start()
    Process(target=node_keeper_client, args=(NODE_KEEPER_IP_2, NODE_KEEPER_CLIENT_REP_2,)).start()
    Process(target=node_keeper_client, args=(NODE_KEEPER_IP_3, NODE_KEEPER_CLIENT_REP_3,)).start()
    Process(target=node_keeper_client, args=(NODE_KEEPER_IP_4, NODE_KEEPER_CLIENT_REP_4,)).start()
